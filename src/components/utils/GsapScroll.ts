...
type EmissiveMesh = THREE.Mesh & { material: THREE.MeshStandardMaterial };

// setCharTimeline is re-run on resize; keep a handle on the previous flicker
// interval so re-invocations don't stack timers.
...
export function setCharTimeline(
  ...
  let screenLight: EmissiveMesh | undefined;
  let monitor: EmissiveMesh | undefined;
  character?.children.forEach((object) => {
    if (object.name === "Plane004") {
      object.children.forEach((child) => {
        const mesh = child as EmissiveMesh;
    ...
    if (object.name === "screenlight") {
      const mesh = object as EmissiveMesh;
      mesh.material.transparent = true;
      mesh.material.opacity = 0;
